...
const Generated = () => {
  ...
  const getProposalList = (e) => {
    ...
    if (searchData && Object.values(searchData).length > 0) {
      remainApiPath = `tl/getPaymentDetail?&invoice=1&page=${e}&orderby=${orderBy}&orderbyfield=${fieldBy}&qno=${searchData.query_no}&from=${searchData.p_dateFrom}&to=${searchData.p_dateTo}&status=${searchData.payment_plan}&installment_no=${searchData?.installment_no}&payment_plan=${searchData.payment_plan}`;
    } else {
  ...
  const sortMessage = (val, field) => {
    ...
    if (data && Object.values(data).length > 0) {
      remainApiPath = `tl/getPaymentDetail?&invoice=1&qno=${data.query_no}&payment_plan=${data.payment_plan}&from=${data.p_dateFrom}&to=${data.p_dateTo}&status=${data.payment_plan}&installment_no=${data?.installment_no}&orderby=${val}&orderbyfield=${field}`
    }
    else {
