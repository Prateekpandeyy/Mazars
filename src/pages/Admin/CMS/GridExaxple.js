...
const GridExample = () => {
  ...
  const getList = () => {
    axios
      .get(`${baseUrl}/cms/getalllinks?uid=${JSON.parse(userId)}`, myConfig)
      .then((res) => {
        if (res.data.code === 1) {
  ...
  const [columnDefs] = useState([
    ...
    },
    {
      field: "Website",
      initialWidth: 500,
      cellRendererFramework: (params) => <p>{params.data.heading}</p>,
    },
  ...
  function del(id) {
    Swal.fire({
      ...
      if (result.value) {
        axios
          .get(
            `${baseUrl}/cms/removelinks?uid=${JSON.parse(userId)}&id=${id}`,
            myConfig
          )
          .then((res) => {
            if (res.data.code === 1) {
  ...
  return (
    <>
      ...
      <button className="autoWidthBtn my-5" onClick={() => allLinkOrder()}>
        Set order
      </button>
    </>
  );
