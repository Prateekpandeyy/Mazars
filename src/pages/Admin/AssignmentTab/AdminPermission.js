...
function AdminPermission(props) {
  ...
  const onSubmit = (data) => {
    if (status.length > 0) {
      axios
        .get(
          `${baseUrl}/admin/getadminpermissiona?cat_id=${store2}&from=${data.p_dateFrom}&to=${data.p_dateTo}&assignment_status=${status}&stages_status=${data.p_status}&pcat_id=${selectedData}`,
          myConfig
        )
        .then((res) => {
          if (res.data.code === 1) {
