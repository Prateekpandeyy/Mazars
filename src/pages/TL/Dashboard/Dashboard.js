...
function Dashboard() {
  ...
  const [issue_proposal, setIssueProposal] = useState("");
  ...
  useEffect(() => {
    const getAllQueries = () => {
      axios
        .get(
          `${baseUrl}/tl/totalComplete?tl_id=${JSON.parse(userid)}`,
          myConfig
        )
        .then((response) => {
    ...
    const getAssignment = () => {
      axios
        .get(
          `${baseUrl}/tl/getAssignmentsCount?tl_id=${JSON.parse(userid)}`,
          myConfig
        )
        .then((response) => {
          if (response.data.code === 1) {
    ...
    const getPayment = () => {
      axios
        .get(
          `${baseUrl}/tl/getAssignmentsPaymentCount?tl_id=${JSON.parse(
            userid
          )}`,
          myConfig
        )
