...
function Tds(props) {
  ...
  const getServices = () => {
    axios.get(`${baseUrl}/tl/getServices`, myConfig).then((res) => {
      if (res.data.code === 1) {
  ...
  useEffect(() => {
    if (props.generated == "edited") {
      ...
      setTotal(
        parseInt(props.paidAmount) +
          parseInt((props.paidAmount * cgetRate) / 100)
      );
      setgrandTotal(
        parseInt(
          parseInt(props.paidAmount) + parseInt((props.paidAmount * 0) / 100)
        ) - parseInt((props.paidAmount * tdsRate) / 100)
      );
    }
  }, [props]);
  const getDataild = () => {
  ...
  const basicFun = (e) => {
    ...
    if (e.target.value > 0) {
      ...
      setGst((oldData) => {
        return (
          parseInt(Math.round((a * cgetRate) / 100)) +
          parseInt(Math.round((a * igetRate) / 100)) +
          parseInt(Math.round((a * sgetRate) / 100))
        );
      });
      setTotal((oldData) => {
        return (
  ...
  const onSubmit = (value) => {
    ...
    {
      props.generated == "edited"
        ? formData.append("generate_status", 1)
  ...
  return (
    <Modal
      isOpen={props.tdsForm}
      toggle={props.addTdsToggle}
      size="sm"
      style={{ display: "flxe", maxWidth: "600px" }}
    >
          ...
          <div className="container gutter-3">
            ...
            <div className="row my-2">
              {services === undefined ? (
                ...
                <div className="col-md-6">
                  <label>
                    Description <span className="declined">*</span>
                  </label>
                  <select
                    value={description}
            ...
            <>
              {loading ? (
                ...
                <>
                  ...
                  <button
                    type="button"
                    className="customBtn mr-auto ml-3"
                    onClick={props.addTdsToggle}
                  >
                    Cancel
                  </button>
                </>
