...
function CompleteAssignment() {
  ...
  function headerLabelFormatter(column, colIndex) {
    ...
    if (
      localStorage.getItem("custArrowAs3") === column.dataField ||
      localStorage.getItem("prevcustAs3") === column.dataField
    ) {
      isActive = true;
      setPrev(column.dataField);
      localStorage.setItem("prevcustAs3", column.dataField);
    } else {
      isActive = false;
    }
    return (
    ...
    },
    {
      ...
      onSort: (field, order) => {
        ...
        if (accend !== field) {
          setAccend(field);
          setIsActive(field);
          localStorage.setItem("custArrowAs3", field);
        } else {
          setAccend("");
