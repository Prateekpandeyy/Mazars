...
const PayModal = (props) => {
  ...
  useEffect(() => {
    if (props.modalData !== undefined) {
      ...
      setpayValue(
        parseInt(props.modalData.invoice_amount) -
          parseInt(props.modalData.tds_amount)
      );
    }
  }, [props.modalData]);
  ...
  const inVal = (e) => {
    if (e.target.value.length > 5) {
      ...
      if (e.target.value > 100) {
        setTdsRate(100);
        let tdAmount = parseInt(
          parseInt(props.modalData.paid_amount) +
            parseInt(props.modalData.opt_expenses)
        );

        let a =
          parseInt(props.modalData.cgst_amount) +
          parseInt(props.modalData.sgst_amount) +
          parseInt(props.modalData.igst_amount);
  ...
  return (
    ...
    >
      ...
      <ModalBody
        style={{ display: "flex", flexDirection: "column", overflow: "wrap" }}
      >
        ...
        <ModalFooter>
          <button onClick={() => payFun()} className="customBtn">
            Pay Now
          </button>
        </ModalFooter>
      </ModalBody>
